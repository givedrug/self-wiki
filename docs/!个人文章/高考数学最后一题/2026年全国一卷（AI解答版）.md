...
第二步的反证，假设有 $a>0$ 使 $f(a)>f(0)$中，$x_2$取的是$0$，$\Delta$取的是$(0,\min\{a,1\})$，$x_1$取的$-a+\Delta$，$d$取的是$a$。假设有 $a>0$ 使 $f(a)=f(0)$中，$x_2$取的是任意负数，$\Delta$取的是$(0,\min\{a,1\})$，$x_1$取的$x_2-a+\Delta$，$d$取的是$a-x_2$。（其实这一步都可以用后面这个方式构造）

第三步有问题，应为证$x>=1$时（而非$0$）的结论，否则取 $y<0$，使 $2^y<f(a)$，无法取到。取$x_2=a$,一定存在$x_1<0$，使得$f(x_1)<f(a)$，取$d=-a$。
